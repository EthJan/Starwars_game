...
from MenuScreens import fullscreen, resolution, DARK_GRAY
...
def play_game():
    ...
    if fullscreen:
        win = pygame.display.set_mode(resolution, pygame.FULLSCREEN)
    else:
    ...
    class player():  # new Player Class
        ...
        shotLeft = pygame.image.load('CLShot-1.png')
    ...
    def spawnEnemy():
        while True:
            ...
            # Player is on the left, spawn enemy on the right (off-screen)
            else:
    ...
    def drawMap():  # Draw color grid
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                # if Map[row][col] == 'x':
                # Even Col and even Row
                # if (col + 1) % 2 == 0:
                #     if (row +1) % 2 == 0:
                #         pygame.draw.rect(win, (60, 60, 60), pygame.Rect(
                #             x, 250 + ((row-1) * 60), Grid_Size, Grid_Size))
                #     elif (row +1) % 2 == 1:
                #         pygame.draw.rect(win, (30, 30, 30), pygame.Rect(
                #         x, 250 + ((row-1) * 60), Grid_Size, Grid_Size))
                # elif (col + 1) % 2 == 1:
                #     if (row +1)%2 ==0:
                #         pygame.draw.rect(win, (30, 30, 30), pygame.Rect(
                ...
                #         x, 250 + ((row-1) * 60), Grid_Size, Grid_Size))
                # if Map[row][col] == 'x':
                color = (110, 110, 110) if (
                    col + row) % 2 == 0 else (170, 170, 170)
                pygame.draw.rect(win, color, pygame.Rect(
                    x, 250 + ((row - 1) * 60), Grid_Size, Grid_Size))

                # Self Coordinate Test Grids
                # if Map[row][col] == 'o':
    ...
    def drawWindow():
        ...
        # UI
        userinterface = (0, 360, WIDTH, 200)
        # placeholder button, need to create actual ones
        button1 = (10, 430, 45, 45)
        pygame.draw.rect(win, DARK_GRAY, userinterface)
    ...
    # Shield Activation
        if shieldActivated:
            if currentTime - shieldstartTime >= shieldelapsedTime:
                shieldActivated = False
...
def main():
    from MenuScreens import main_menu
    while True:
        main_menu(play_game)
